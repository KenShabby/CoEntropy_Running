...
class IntervalSegment(RunSegment):
    def __init__(
        self, effort_name, reps, effort_pct, rest_pct, effort_duration, rest_duration
    ):
        super().__init__(
            effort_name,
            reps * (effort_duration + rest_duration),
            effort_pct,
            effort_pct,
        )
    ...
    def describe(self, runner):
        ...
        rest_low, rest_high = reserve_hr(runner, self.rest_pct, self.rest_pct)
